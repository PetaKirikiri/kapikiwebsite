...
export class KitchenMovement {
 ...
 tick(authoritative:KitchenPlayer,direction:Point & {dash?:boolean},dt:number,now:number){
  if(!this.local)this.local={...authoritative,...positionAt(authoritative,now),path:[],target:null}
  if(!this.running&&!this.pending.length)this.local={...authoritative,path:[],target:null}
 ...
 reset(){this.generation++;this.local=null;this.pending=[]}
