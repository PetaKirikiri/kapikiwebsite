...
describe('local-first kitchen interactions',()=>{
 it('picks up, chops, collects and places before the first database reply',()=>{
  ...
  expect(q.view!.stations['chop-a'].readyAt).toBe(4700)
  const collect=activate(q,server,4,1,4800)
  expect(q.view!.players[0].held).toBe('chopped:tomato')
  const place=activate(q,server,4,4,5500)
  ...
  expect(q.view!.stations['counter-4'].item).toBe('chopped:tomato')
  confirm(server,pickup.id!,1,1,1000,6000);q.reconcile(server,0)
  expect(q.saving).toBe(true);expect(q.view!.players[0].held).toBeNull()
  confirm(server,chop.id!,4,1,1700,6100);q.reconcile(server,0)
  expect(server.stations['chop-a'].readyAt).toBe(4700)
  confirm(server,collect.id!,4,1,4800,6200)
  confirm(server,place.id!,4,4,5500,6300);q.reconcile(server,0)
 ...
 it('bounds client action timestamps and rejects malformed times',()=>{
  ...
  expect(server.stations['chop-a'].readyAt).toBe(11000)
 ...
 it('pauses work locally and replays the pause before a delayed server reply',()=>{
  ...
  expect(pauseId).toBeTruthy();expect(q.view!.stations['chop-a'].remainingMs).toBe(2000)
  ...
  expect(server.stations['chop-a'].remainingMs).toBe(2000)
  const resume=activate(q,server,4,1,8000)
  expect(resume.id).toBeTruthy();expect(q.view!.stations['chop-a'].readyAt).toBe(10000)
 })
